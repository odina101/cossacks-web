	...
	int NActive, char** Active, int NParam, char** Param, char* param )
{
	...
	DSS->addInputBox( nullptr, *x, *y, VAR, 500, ( *x1 ) - ( *x ), ( *y1 ) - ( *y ), SXP->GetFontByName( SXP->FONT1 ), SXP->GetFontByName( SXP->FONT3 ) );
	return true;
};
struct PingST
{
	...
	int NActive, char** Active, int NParam, char** Param, char* param )
{
	...
	*x1 += OSW->sstb_wholedx;
	...
	GPB = DSS->addGP_TextButton( nullptr, x0 + OSW->sstb_dx, y0 + OSW->sstb_dy, Param[0], sbgp, OSW->sstb_Index + 1, AFONT, PFONT );
	...
	int NActive, char** Active, int NParam, char** Param, char* param )
{
	...
	if (NParam > 1)
	{
		...
		RLCFont* AFONT = SXP->GetFontByName( SXP->FONT2 );
		RLCFont* MFONT = SXP->GetFontByName( SXP->FONT3 );
		DSS->addTextButton( nullptr, ( *x + *x1 ) / 2, ( ( *y + *y1 - GetRLCHeight( PFONT->RLC, 'W' ) ) / 2 ) - 1, Param[1], AFONT, PFONT, MFONT, 1 );
	};
	if (NActive)
	{
						...
						if (p1 < 60)
						{
							...
							if (s2)
							{
								...
								if (GP_Idx != -1)
								{
									...
									GP_Button* GB = DSS->addGP_Button( nullptr, cury + gpdy + ( thisLy - Gly ) / 2, curx + gpdx + ( LXI[p] - Glx ) / 2, GP_Idx, a_sp, p_sp );
									SD = GB;
								};
							};
						};
	...
	int NActive, char** Active, int NParam, char** Param, char* param )
{
	...
	for (int i = 0; i < ADI->NColumns; i++)if (ADI->COLM[i].Width)
	{
		...
		if (ADI->Style & 1)
		{
			...
			CheckBox* CB;
			CB = DSS->addGP_CheckBox( nullptr, xx + OSW->b_DxL, ( *y ) + OSW->b_DyL, ADI->COLM[i].Message, AFONT, PFONT, 1, 0, CurPalette ? IBOR2 : IBOR0, 26, 23, 20 );
			CB->Central = 1;
		...
		if (ADI->Style & 2)
		{
			...
			if (ft > 1 && ft <= 4)
			{
				ComboBox* CBB = DSS->addGP_ComboBoxDLX( nullptr, xx + OSW->b_DxL, ( *y1 ) - OSW->CTBL_LowLY + 1, xx1 - xx - OSW->b_DxR - OSW->b_DxL + dw/*+24-OSW->cmb_bdx*/, comidx, OSW->cmb_Idx, 9, 0, AFONT, PFONT, ADI->COLM[i].FiltParam );
	...
	if (scidx != -1)
	{
		VScrollBar* VS = DSS->addNewGP_VScrollBar( nullptr, ( *x1 ) - OSW->sb_DxR + OSW->sc_dx, TY0 + OSW->sb_DyL + OSW->sc_dy, TY1 - TY0 + 1 + OSW->sc_dly - OSW->sb_DyL - OSW->sb_DyR, 100, 0, scidx, OSW->scr_Idx );
		VS->ScrDy = 6 * 10;
		VS->OnesDy = 10;
		CBX->VS = VS;
		ADI->VS = VS;
