...
bool GP_TextButton_OnDraw( SimpleDialog* SD )
{
	...
	{
		if (tb->Sprite1 != -1)for (int q = 0; q < tb->Nx; q++)GPS.ShowGP( tb->x + q*tb->OneLx, tb->y, tb->GP_File, tb->Sprite1, 0 );
	};
	//drawing text
	RLCFont* CFN = nullptr;
	if (!tb->Enabled&&tb->DisabledFont)CFN = tb->DisabledFont;
	...
	if (CB->IsActive)
	{
		...
		for (int i = FL; i < LL; i++)
		{
			if (i == CB->LightIndex)
			{
				...
				ShowString( CB->x + CB->FontDx + center*( ( CB->x1 - CB->x - GetRLCStrWidth( CB->Lines[i], CB->ActiveFont ) ) >> 1 ), CB->DropY + CB->FontDy + ( i - FL )*CB->OneLy + CB->UpLy, CB->Lines[i], CB->ActiveFont );
			}
			else ShowString( CB->x + CB->FontDx + center*( ( CB->x1 - CB->x - GetRLCStrWidth( CB->Lines[i], CB->PassiveFont ) ) >> 1 ), CB->DropY + CB->FontDy + ( i - FL )*CB->OneLy + CB->UpLy, CB->Lines[i], CB->PassiveFont );
			y0 += CB->OneLy;
		};
		if (CB->VS&&CB->NLines > CB->MaxLY)
		{
...
{
	*SCR = DSS->addNewGP_VScrollBar( nullptr, x1 - DSS->BaseX - GPS.GetGPWidth( GPFile, Sprite ) + 1, y1 + 5 - DSS->BaseY, OneLy*MaxLy + 9, 1, 0, GPFile, Sprite );
	VS = *SCR;
	VS->Visible = 0;
	VS->Enabled = 0;
	VS->SPos = 0;
